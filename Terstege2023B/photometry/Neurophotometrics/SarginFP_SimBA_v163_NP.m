...
%           Identify animal ID
%           Identify Neurophotometrics output files
%           Identify SimBA output files
%           Number of frames in the dataset

%internal initialization
for ii = 1:FP.info.groupnum(1)
    %create structure arrays for raw np timestamp inputs
    %adjust the number of frames (default = 180705, aka 25mins @120fps) as needed
    FP.inputs.np.time.(FP.info.groups(ii))=zeros(180705,FP.info.pergroup(ii)); %#frames
    %create structure arrays for raw np calcium signal inputs
    %adjust the number of frames (default = 180705, aka 25mins @120fps) as needed
    FP.inputs.np.sig.(FP.info.groups(ii))=zeros(180705,FP.info.pergroup(ii)); %#frames
    %create structure arrays for raw np isosbestic inputs
    %adjust the number of frames (default = 180705, aka 25mins @120fps) as needed
    FP.inputs.np.iso.(FP.info.groups(ii))=zeros(180705,FP.info.pergroup(ii)); %#frames
...
for ii=1:FP.info.groupnum(1)
    for iii=1:FP.info.pergroup(ii)
        %import raw data
        prompt=strcat("Select Neurophotometrics 488 file for mouse #",num2str(iii)," in ",FP.info.groups(ii)," group:");
        disp(prompt);
        [npfile,pathname]=uigetfile(('*.csv'),prompt); %load np file
        sigdata=readmatrix(strcat(pathname,npfile));
        %trim if exceeds maximum trial duration (default = 180705, aka 25mins
        %@120fps)
        if size(sigdata,1)>180705 %#frames
            sigdata=sigdata(1:180705,:); %#frames
        else
            %do nothing
        end
        prompt=strcat("Select Neurophotometrics 415 file for mouse #",num2str(iii)," in ",FP.info.groups(ii)," group:");
        disp(prompt);
        [npfile,pathname]=uigetfile(('*.csv'),prompt); %load np file
        isodata=readmatrix(strcat(pathname,npfile));
        %trim if exceeds maximum trial duration (default = 180705, aka 25mins
        %@120fps)
        if size(isodata,1)>180705 %#frames
            isodata=isodata(1:180705,:); %#frames
        else
            %do nothing
        end
        ...
        %organize raw data
        %adjust the number of frames (default = 180705, aka 25mins @120fps) as needed
        FP.inputs.np.time.(FP.info.groups(ii))(1:size(sigdata,1),iii)=sigdata(:,2); %timestamps from np output
        FP.inputs.np.sig.(FP.info.groups(ii))(1:size(sigdata,1),iii)=sigdata(:,9); %calcium-dependent signal from np output
        FP.inputs.np.iso.(FP.info.groups(ii))(1:size(isodata,1),iii)=isodata(:,9); %isosbestic sigal from np output
        %adjust the number of frames (default = 6000, aka 5mins @20fps) as needed
...
clear ii iii prompt pathname npfile familiarfile intruderfile
clear isodata sigdata familiardata intruderdata famtrim inttrim firstmouse

%process successful
disp('Process 2. Initialization - Subject Information Complete');


%%  03. Photometry Processing - Behaviour Independent
...
for ii=1:FP.info.groupnum(1)
    ...
    for iii=1:FP.info.pergroup(ii)
        %full trace
        time=FP.inputs.np.time.(FP.info.groups(ii))(:,iii);
        sig=FP.inputs.np.sig.(FP.info.groups(ii))(:,iii);
        iso=FP.inputs.np.iso.(FP.info.groups(ii))(:,iii);
        temp_x=1:length(iso);
...
%%  04. Photometry Processing - Behaviour Epoch Dependent
%   
...
%%  05. Photometry Processing - Bout-by-Bout Information
